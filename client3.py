    ...
    def opened(self):
        ...
        def send_data_to_ws():
            if self.send_adaptation_state_filename is not None:
                print(48, "Sending adaptation state from %s" % self.send_adaptation_state_filename)
                try:
                    adaptation_state_props = json.load(open(self.send_adaptation_state_filename, "r"))
                    self.send(json.dumps(dict(adaptation_state=adaptation_state_props)))
                except:
                    e = sys.exc_info()[0]
                    print(54, "Failed to send adaptation state: ",  e)
            with self.audiofile as audiostream:
                for block in iter(lambda: audiostream.read(int(self.byterate/4)), ""):
                    self.send_data(block)
            print(58, "Audio sent, now sending EOS")
            self.send("EOS")
        ...
        if response['status'] == 0:
            if 'result' in response:
                ...
                if response['result']['final']:
                    ...
                    print(75, '\r%s' % trans.replace("\n", "\\n"))
                else:
                    print_trans = trans.replace("\n", "\\n")
                    if len(print_trans) > 80:
                        print_trans = "... %s" % print_trans[-76:]
                    print(80, '\r%s' % print_trans,)
            if 'adaptation_state' in response:
                if self.save_adaptation_state_filename:
                    print(83, "Saving adaptation state to %s" % self.save_adaptation_state_filename)
                    with open(self.save_adaptation_state_filename, "w") as f:
                        f.write(json.dumps(response['adaptation_state']))
        else:
            print(87, "Received error from server (status %d)" % response['status'])
